/* Scoreboard loader with AUTO-SKIN v3 (adds HEADER styling).
   - JSON-first (no CSV 404s)
   - Scoped CSS under body[data-cl-skin="scoreboard"]
   - Includes explicit .site-header / .wrap / .nav styles to match other pages
*/
(function(){
  'use strict';
  ...
  function injectSkin(){
    ...
    const css = `
    /* ===== Header (scoped) ===== */
    body[data-cl-skin="scoreboard"] .site-header{
      position: sticky;
      top: 0;
      height: var(--h, ${SKIN.headerH});
    ...
    body[data-cl-skin="scoreboard"] .site-header .wrap{
      max-width: 1200px;
      margin: 0 auto;
      display: grid;
    ...
    body[data-cl-skin="scoreboard"] .site-header .brand{
      font-weight: ${SKIN.brandWeight};
      letter-spacing: .4px;
    }
    body[data-cl-skin="scoreboard"] .site-header .nav a{
      color: ${SKIN.navDim};
      text-decoration: none;
      margin-left: 16px;
      font-weight: 600;
    }
    body[data-cl-skin="scoreboard"] .site-header .nav a:hover{
      color: ${SKIN.navColor};
    }
    body[data-cl-skin="scoreboard"] .site-header .nav a.active{
      color: ${SKIN.navActive};
    }

    /* ===== Panels / Cards / Tables (scoped) ===== */
    ...
    // Console hint: did we see a header?
    const header = document.querySelector('.site-header');
    if (!header) {
      console.info('[CapLeague] No .site-header found. Auto-skin header rules loaded but not applied.');
    } else {
      console.info('[CapLeague] .site-header found; auto-skin header rules active.');
    }
  }
